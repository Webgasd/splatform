...
class DrugProduction extends Component{
    ...
    changeStartTime=(value,option,index)=>{
        let drugsProduceList = this.props.input.drugsProduceList||[];
        drugsProduceList[index][option]=value;
        drugsProduceList[index].giveTime=value;
        drugsProduceList[index].endTime=moment(value).add(5,'years').subtract(1,'day')
    ...
    render() {
        ...
        const drugsProduceList = this.props.input.drugsProduceList||[]
        const index =this.props.index
        const formData =drugsProduceList[index]||{}
        ...
        return (
            <div>
                ...
                <table>
                    ...
                    <tr>
                        <td>企业负责人</td>
                        <td ><Input value={formData.enterprisePerson} onChange={(e)=>this.changeInput(e.target.value,"enterprisePerson",index)} placeholder={"请输入企业负责人"} disabled={checkStatus}/></td>
                        <td>质量负责人</td>
                        <td ><Input value={formData.qualityPerson} onChange={(e)=>this.changeInput(e.target.value,"qualityPerson",index)} placeholder={"请输入质量负责人"} disabled={checkStatus}/></td>
                    </tr>
                    <tr>
                        <td>许可证号<span style={{color:'#FF3300'}}>*</span></td>
                        <td><Input value={formData.number} onChange={(e)=>this.changeInput(e.target.value,"number",index)} placeholder={"请输入证号"} disabled={checkStatus}/></td>
                        <td>分类码</td>
                        <td><Input value={formData.operationMode} onChange={(e)=>this.changeInput(e.target.value,"operationMode",index)} placeholder={"请输入分类码"} disabled={checkStatus}/></td>
                    </tr>
                    <tr>
                        <td >生产范围</td>
            ...
            {/* 检查及评定 */}
                ...
                <tbody>
                    <tr>
                        ...
                        <td style={{width:'3%'}}><img src={require("./imgs/药品生产.png")} style={{height:'8%'}} alt=""/></td>
                        <td style={{width:'30%'}}><Checkbox.Group style={{width:'100%',textAlign:"left"}} value={formData.checkType?formData.checkType.split(','):[]}
                            onChange={(value)=>this.onCheckChange(value,'checkType',index)} disabled={checkStatus}>
                                ...
                                <Checkbox style={{marginLeft:5}} value={'药品生产'}>药品生产</Checkbox>
