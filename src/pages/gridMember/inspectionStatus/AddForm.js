...
class AddForm extends Component {
    ...
    render() {
        ...
        const { InspectionType, mockData, inspectionHandle, InfoValidity } = this.props.state
        //转换返回的文件字段格式  转了需要使用
        ...
        return (
            <div>
                <Row style={{ marginTop: 10 }}>
                    ...
                    <Col span={5}><Input placeholder='请输入内容' value={informData.grider || ''} onChange={(e) => this.changeInput(e.target.value, 'grider')} disabled={true} /></Col>
                    <Col span={3} style={{ textAlign: 'right', fontSize: 15 }}>上报日期：</Col>
                    <Col span={5}><DatePicker value={informData.issueDate == undefined ? null : moment(informData.issueDate, dateFormat)} format={dateFormat} onChange={(dataString) => this.changeInput(dataString, 'issueDate')} disabled={true} /></Col>
                    <Col span={3} style={{ textAlign: 'right', fontSize: 15 }}>问题类型：</Col>
                    <Col span={5}>
                        <Select value={informData.type} style={{ width: 120 }} onChange={(value) => this.changeInput(value, 'typeId')} disabled={true} >
                ...
                <Row style={{ marginTop: 10 }}>
                    ...
                    <Col span={13}><Input placeholder='请输入内容' value={informData.inspectTitle || ''} onChange={(e) => this.changeInput(e.target.value, 'inspectTitle')} disabled={true} /></Col>
