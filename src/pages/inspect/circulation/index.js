...
@connect(
    state=>({
        ...
        acl:state.acls['/inspect/circulation']
    })
)
class restaurantscheck extends Component{
    ...
    render() {
        ...
        const modalFooter = <div>
            {this.state.type=='create'?null:<Button type="primary" style={{marginLeft:5}} onClick={()=> window.open(commonUrl+'/inspect/dailyFood/pointDownload?checkId='+this.state.inspectData.id+'&industryId='+3)}>导出检查要点表</Button>}
            {this.state.type=='create'?null:<Button type="primary" style={{marginLeft:5}} onClick={()=> window.open(commonUrl+'/inspect/dailyFood/recordDownload?checkId='+this.state.inspectData.id+'&industryId='+3)}>导出检查记录表</Button>}
            ...
            <div ref='circulationCheck'>
              <Card>
                ...
                <Modal
                    ...
                    destroyOnClose={true}
                    maskClosable={false}
                    getContainer={()=>this.refs.circulationCheck}
                    width={900}
                    footer={modalFooter}
                    onCancel={()=>{
                        this.setState({
