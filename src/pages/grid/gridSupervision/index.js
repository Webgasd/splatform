...
import {Row, Col, Tree, Divider, Card, Button, message,Modal,TreeSelect,Input} from 'antd'
...
import titlegif from './image/动态球.gif'
import titlepng from './image/政府徽标.png'
import star from "./image/bj1.jpg"
///引入图标
import tubiao from "../showGrid/image/pic1.png";
...
class GridSupervision extends React.Component{
    constructor(props){
        super(props)
        this.state={
            visible:false,
    ...
    requestFullScreen = () => {
        ...
        var de =document.getElementById("MainContainer")
    ...
    render() {
        const option4 = {
            ...
            legend: {
                ...
                data:(this.state.NewIndustryList||[]).map((item)=>item.name)||[]
            },
            grid: {
                left: '4%',
                right: '3%',
        ...
        const IndustryList= [{id:1,name:'食品行业'},
                {id:2,name:'药品行业'},
                {id:3,name:'化妆品行业'}]
        const option3 = {
            tooltip : {
        ...
        const option = {
            ...
            radar: {
                ...
                indicator: this.state.NewIndustryList?this.setRadarLegend():[]
            },
            series: [{
                name: '预算 vs 开销（Budget vs spending）',
                type: 'radar',
                areaStyle: {normal: {}},
                data : this.state.PointsList.length>0?this.getRandarValue():[]
            }]
        };
        const modal=(<Modal
            title="企业信息"
            ...
            destroyOnClose
             getContainer={()=>this.refs.gridSupervisionCard}
            centered={true}
            width={1000}
            onCancel={()=>{
        ...
        const ledgerTable=(
            ...
            return(
                <Row id={'MainContainer'} className={'MainContainer'}>
                    <div className={'HorizontalLine'}/>
                    <div className={'LeftVerticalLine'}/>
                    <div className={'RightVerticalLine'}/>
                    <Row className={'GridTitleTop'}>
                        <Col span={1} className={'TitleFirst'}><img src={titlegif} style={{width:'90px',height:'50px'}}/></Col>
                        <Col span={4} className={'TitleSecond'}>
                            <Col span={10}><text>区域选择</text></Col>
                            <Col span={14}>
                                <div className={'gridTreeSelect'}>
                                    <TreeSelect
                                        style={{ width: '100%',height:'100%' }}
                                        // value={this.state.gridSelectValue}
                                        dropdownStyle={{ overflow: 'none',backgroundColor:'rgba(255,255,255,0.3)' }}
                                        allowClear
                                        // treeDefaultExpandAll
                                        onSelect={this.onSelect}
                                        placeholder={"请选择"}
                                    >
                                        {this.renderTreeNodes(this.state.areaList||[])}
                                    </TreeSelect>
                                </div>
                            </Col>
                        </Col>
                        <Col span={8} className={'TitleThird'}>
                            <img src={titlepng} style={{width:'50px',height:'50px'}}/>
                            <text>{unitName}市场监督管理局地图网格化监督</text>
                        </Col>
                        <Col span={4} className={'TitleForth'}>
                            <Col span={10}><text>行业选择</text></Col>
                            <Col span={14} className={'gridTreeSelect'}>
                                <TreeSelect
                                    // value={this.state.selectIndustryList}
                                    style={{ width: '100%',height:'100%' }}
                                    dropdownStyle={{ overflow: 'none',backgroundColor:'rgba(255,255,255,0.3)' }}
                                    allowClear={()=>console.log("asd")}
                                    // treeDefaultExpandAll
                                    onChange={this.onSelectType}
                                    placeholder={"请选择"}
                                >
                                    {/*{this.renderTypeTreeNodes(this.state.IndustryType||[])}*/}
                                    {this.renderTypeTreeNodes(IndustryList||[])}
                                </TreeSelect>
                            </Col>
                        </Col>
                        <Col span={2} className={'TitleFifth'}>
                            <button onClick={()=>this.fullScreen()}>切换全屏</button>
                        </Col>
                    </Row>
                    <Row className={'MustMainContainer'}>
                        <Col span={16}>
                            <Row>
                                <Col span={6} push={2}>
                                    <div className={'leftNumberInfo'}>
                                        <div className={'firstInfo'}><text>辖区企业（家）</text></div>
                                        <div className={'secondInfo'}><text>{this.state.PointsListLength}</text></div>
                                    </div>
                                    <div className={'leftNumberInfo'}>
                                        <div className={'thirdInfo'}><text>异常企业（家）</text></div>
                                        <div className={'forthInfo'}><text>0</text></div>
                                    </div>
                                </Col>
                                <Col span={18}>
                                    <div className={'gridsu-warp'}>
                                        <div className={"filterCard"}>
                                            <text>查询企业</text>
                                            <input  onChange={(e)=>this.setState({searchEnterprise:e.target.value})}/>
                                            <Button onClick={()=>this.searchEnterprise()}>查询</Button>
                                        </div>
                                        <div ref={'gridSupervisionCard'} id={'gridSupervision'} className={'gridSupervision'} >
                            ...
                            <Row>
                                <Col>
                                    <div className={'leftBottom'}>
                                        <Card title={this.getTitle("各执法单位管辖情况")} type={'inner'} className={'leftBottomCard'}>
                                            <ReactEcharts
                                                option={option4}
                                                theme="UPC"
                                                notMerge={true}
                        ...
                        <Col  span={7}>
                            <div id={'rightTop'} className={'rightTop'}>
                                <Card title={this.getTitle("辖区业态分布")} type={'inner'} className={'rightTopCard'}>
                                    <ReactEcharts
                            ...
                            <div id={'rightBottom'} className={'rightBottom'}>
                                <Card title={this.getTitle("证件预警情况")} type={'inner'} className={'rightBottomCard'}>
                                    <ReactEcharts
                                        option={option}
                                        theme="UPC"
                                        notMerge={true}
...
export default GridSupervision