...
class DetailForm extends Component {
    ...
    changeDoubleInput1 = (value, option, value1, option1) => {
        ...
        return (
            ...
            <th style={{width:100}}>原料类型</th>  
                ...
                <td style={{width:250}}><Select value={item.originType} style={{width:"100%"}} onChange={(value)=>this.changeInput(value,"originType")} disabled={type=='detail'?true:false}>
                           ...
                           <th >备案日期</th>
                <td ><DatePicker style={{width:'100%'}} value={item.recordTime=moment(item.recordTime)} onChange={(date)=>this.changeInput(date,"recordTime")}disabled={type=='detail'?true:false} format="YYYY-MM-DD"/></td>
                </tr>
                <tr>
                ...
                {/* <td><Input  style={{width:250}}value={item.originName} onClick={()=>this.setState({
                    ...
                    suffix={<Icon type="search" />}disabled={type=='detail'?true:false}/></td> */}
                      <td><Input style={{width:250}} value={item.originName } onChange={(e)=>this.changeInput(e.target.value,"originName")}disabled={type=='detail'?true:false}/></td>
                </td>
                <th style={{width:100}}>原料类别</th>
                {/* <td><Input placeholder="自动生成" value={item.originTypeEx }disabled={type=='detail'?true:false}/></td> */}
                  <td><Input value={item.originTypeEx} onClick={()=>this.setState({
                    isVisible:true, personType:'originTypeEx'})} 
                    suffix={<Icon type="search" />}disabled={type=='detail'?true:false}/></td>
                </tr>
                <tr>
                <th style={{width:100}}>生产商</th>
                <td><Input value={item.producer} onChange={(e)=>this.changeInput(e.target.value,"producer")} disabled={type=='detail'?true:false}/></td>
                <th>品牌</th>
            ...
            <tr>
                ...
                <th>有效期限</th>
                <td><DatePicker style={{width:'100%'}} value={item.deadTime=moment(item.produceTime).add(item.keepTime,'days')} onChange={(date)=>this.changeInput(date,"deadTime")}disabled={type=='detail'?true:false} format="YYYY-MM-DD"/></td>
            ...
            <tr>
               ...
               <th>供应商</th>
            <td><Input value={item.supplier} onClick={type=='detail'?(e)=>this.setState({isSupVisible:true,personType:'supplier',sup:item.id}):()=>this.setState({isEvisible:true,personType:'supplier',eType:'supplierType'})} suffix={<Icon type="search" />} /></td>
            </tr>
            {/* <tr>  
                <th>出货数量</th>
            ...
            <tr>
                ...
                <td>
                    ...
                    {/* <Input value={item.person}  onChange={(e)=>this.changeInput(e.target.value,"person")} disabled={type=='detail'?true:false}/> */}
                    <Input value={item.person} onClick={()=>this.setState({
                    isVisible:true, personType:'person'})} 
                    suffix={<Icon type="search" />}disabled={type=='detail'?true:false}/>
                    </td>

            </tr>
            {/* <tr> */}
                {/* <th style={{width:100}}>供应商类型</th> */}
                <Input
                 type="hidden" 
                 value={item.supplierType} onChange={(e)=>this.changeInput(e.target.value,"supplierType")}/>
            {/* </tr> */}
            </tbody>
        </table>
        </div>
            ...
            <Table
                ...
                >
                    ...
                    <SupervisorForm 
                    dispatchSupervisor={(data1,data2)=>{
                        this.setState({isEvisible:false})
                        this.changeDoubleInput1(data1,this.state.personType,
                            data2,this.state.eType,
                            );
                    }} 
