...
class DetailForm extends Component {
    ...
    //     }
    //     this.changeList(fileList,"document",index);
    // };
    handleFile = (info) => {
        const fileList = info.fileList;
        if (info.file.status === 'done') {
            ...
            <div>
                ...
                <DatePicker style={{width:'100%'}}
                        disabled={type=='detail'?true:false}
                                        value={formData.recordTime=moment(formData.recordTime)}
        ...
        {list.map((item,index)=>(
           ...
           <td> <Input value={index==0?item.seq=1:item.seq=index+1} onChange={(e)=>this.changeList(e.target.value,"seq",index)} disabled={type=='detail'?true:false}/></td>
            ...
            <th style={{width:60}}>原料类型</th>  
                ...
                {/* <td><Input value={item.produceName} onChange={(e)=>this.changeList(e.target.value,"produceName",index)}disabled={type=='detail'?true:false}/></td> */}
            ...
            <tr>
                <th>进货数量</th>
                <td><Input  value={item.goodsIn} placeholder='请填写数字' onChange={(e)=>this.changeList(e.target.value,"goodsIn",index)}disabled={type=='detail'?true:false}/></td>

                <th>进货计量单位</th>
                {/* <td><Select value={item.goodsInType}style={{width:"100%"}} onChange={(value)=>this.changeList(value,"goodsInType",index)} disabled={type=='detail'?true:false}>
               ...
               <td ><Input value={item.goodsInType} onClick={()=>this.setState({isVisible:true,personType:'goodsInType',index})}disabled={type=='detail'?true:false} suffix={<Icon type="search" />} /></td>
                <th>出货数量</th>
                <td><Input  value={item.goodsOut} placeholder='数字小于进货数'  onChange={(e)=>this.changeList(e.target.value,"goodsOut",index)} disabled={type=='detail'?true:false}/></td>
                <th>库存数量</th>
                <td><Input value={item.goods=item.goodsIn==undefined?0:(item.goodsOut==undefined?0:item.goodsIn-item.goodsOut)}  onChange={(e)=>this.changeList(e.target.value,"goods",index)} disabled={type=='detail'?true:false}/></td>
            </tr>
            <tr>
            <th>供应商</th>
            <td colSpan={3}><Input value={item.supplier} onClick={type=='detail'?(e)=>this.setState({isSupVisible:true,personType:'supplier',sup:e.target.value,index}):()=>this.setState({isVisible:true,personType:'supplier',index})} suffix={<Icon type="search" />} /></td>
                <th>索证索票</th>
                ...
                <th>验收人</th>
                <td><Input value={item.person}  onChange={(e)=>this.changeList(e.target.value,"person",index)} disabled={type=='detail'?true:false}/></td>
            </tr>
            {/* <td> <Upload 
                            name='file'
                            disabled={type=='detail'?true:false}
                            action={commonUrl+'/upload/uploadPicture'}
                            onChange={(info)=>this.handleFile(info,index)}
