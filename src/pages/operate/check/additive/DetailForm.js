...
class DetailForm extends Component {
    ...
    render() {
                ...
                <Card  style={{marginLeft:15,marginRight:15}}>
                    ...
                    {/* <th>使用单位</th>
                        ...
                        disabled={type=='detail'?true:false}
                                        value={formData.giveTime===''?moment():moment(formData.recordDate)}
                                        onChange={(date)=>this.changeInput(date,'recordDate')}
                                        showTime={true}  format="YYYY-MM-DD"/></td>
                       <tr>
                        <th>备案号</th>
                        <td><Input value={formData.recordNo} onChange={(e)=>this.changeInput(e.target.value,"recordNo")} disabled={type=='detail'?true:false}/></td>
                       
                        <th>公示状态</th>
                        <td><Select value={formData.publicity} style={{width:"100%"}} onChange={(value)=>this.changeInput(value,"publicity")} disabled={type=='detail'?true:false}>
        ...
        {list.map((item,index)=>(
                        <tr>
                            <td style={{width:60}}><Input value={item.seq} onChange={(e)=>this.changeList(e.target.value,"seq",index)} disabled={type=='detail'?true:false}/></td>
                            ...
                            <td style={{width:120}}><DatePicker value={item.usedate===''?moment():moment(item.usedate)} onChange={(date)=>this.changeList(date,"usedate",index)}  disabled={type=='detail'?true:false} format="YYYY-MM-DD"/></td>
